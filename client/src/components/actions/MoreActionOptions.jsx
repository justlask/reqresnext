import React from 'react'
import Button from '../Button'

const MoreActionOptions = (props) => {
  const handleComplete = () => {
    return (props.action.complete) ?
      <Button className="noButtonEdit" onClick={e => props.markIncomplete()} title="Mark As Incomplete"></Button>
    :
      <Button className="noButtonEdit" onClick={e => props.markComplete()} title="Mark As Complete"></Button>
  }

  return (!props.show) ? null : (
    <div className="editme">
      <Button className="noButtonEdit" title="Edit this action" onClick={e => props.toggleModal(e)}></Button>
      {handleComplete()}
      <Button className="noButtonEdit" title="Delete this action" onClick={e => props.deleteAction(e)}></Button>
    </div>
  )
}

export default MoreActionOptions
